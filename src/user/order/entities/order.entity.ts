...
export class OrderItem {
    ...
    @ApiProperty({ description: 'Product details for the order item' })
    product: {
      id: string;
      categoryId: string;
      image: string;
    ...
    @ApiProperty({ description: 'Quantity of the product item ordered', example: 2 })
    quantity: number;
  
    @ApiProperty({ description: 'Price of the product item', example: 100 })
    price: number;
  }
